...
class ParroHttp
{
  ...
  function __construct($class)
  {
    ...
    if ($class === 'apiController' && defined('DOING_API')) {
      $this->call         = 'api';
      $this->authenticate = Funciones::parro_api_authentication();
    } elseif ($class === 'ajaxController' && defined('DOING_AJAX')) {
      $this->call = 'ajax';
    } else {
      throw new ParroHttpException(Funciones::get_parro_message(0), 403); // 403
    }
    ...
    if(!in_array(strtoupper($this->r_type), $this->accepted_verbs)) {
      throw new ParroHttpException(Funciones::get_parro_message(1), 403); // 403
    }
  ...
  public function authenticate_request()
  {
    ...
    $api_key = Funciones::get_parro_api_private_key();
    
    if (strcmp($api_key, $this->private_key) !== 0) {
      throw new ParroHttpException(Funciones::get_parro_message(0), 403);
    }
  ...
  private function validate_csrf()
  {
    if ($this->call === 'ajax' && in_array(strtolower($this->r_type), ['post', 'put', 'delete', 'headers', 'options']) && !Csrf::validate($this->csrf)) {
      throw new ParroHttpException('Autorización no válida.', 401); // 401
    }
  }
  ...
  public function accept(Array $verbs)
  {
    if (!in_array(strtoupper($this->r_type), array_map('strtoupper', $verbs))) {
      throw new ParroHttpException('El verbo HTTP utilizado en esta ruta no está autorizado.', 403);
    }
