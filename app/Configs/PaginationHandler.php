...
class PaginationHandler extends Model
{
	...
	public function create_pagination()
	{
		...
		'<li class="page-item '.($this->page == 1 ? 'disabled' : '').'">
			<a class="page-link" href="'.Funciones::buildURL($this->pattern, [$this->variable => $this->page - 1], false, false).'" title="Anterior">&laquo;</a>
		</li>';
		...
		for ($i = 1; $i <= $this->pages; $i++) {
				...
				<li class="page-item '.($this->page == $i ? 'active disabled' : '').'">
					<a class="page-link" href="'.Funciones::buildURL($this->pattern, [$this->variable => $i], false, false).'" >'.$i.'</a>
				</li>
				';
		}
		...
		'<li class="page-item '.($this->page >= $this->pages ? 'disabled' : '').'">
			<a class="page-link" href="'.Funciones::buildURL($this->pattern, [$this->variable => $this->page + 1], false, false).'" title="Siguiente">&raquo;</a>
		</li>';
		$pagination .= '</ul>';
